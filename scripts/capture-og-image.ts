...
function jpegDimensions(bytes: Buffer): { width: number; height: number } {
  if (bytes[0] !== 0xff || bytes[1] !== 0xd8) throw new Error('OUTPUT_NOT_JPEG');
  let offset = 2;
...
function parseArguments(arguments_: string[], repositoryRoot: string): CaptureOptions {
  let outputPath = resolve(repositoryRoot, `public${SITE.defaultOpenGraphImage.path}`);
  let overwrite = false;
  ...
  if (existsSync(outputPath) && !overwrite) throw new Error('CAPTURE_OUTPUT_EXISTS_USE_OVERWRITE');
...
function build(repositoryRoot: string): void {
  ...
  if (result.error || result.status !== 0) throw new Error('CAPTURE_BUILD_FAILED');
}
...
async function waitForPreview(origin: string, preview: ChildProcess): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      lastError = error instanceof Error ? error.message : 'preview request failed';
    }
...
async function stopPreview(preview: ChildProcess | undefined): Promise<void> {
  ...
  await Promise.race([
    new Promise<void>((resolveExit) => preview.once('exit', () => resolveExit())),
...
function verifyOutput(path: string): void {
  ...
  if (
    ...
    throw new Error(`CAPTURE_DIMENSIONS_MISMATCH:${dimensions.width}x${dimensions.height}`);
  }
...
export function replaceValidatedCapture(temporaryPath: string, outputPath: string): void {
  ...
  try {
    ...
    await context.addInitScript(() => {
      ...
      Object.defineProperty(window, '__JET_OG_CAPTURE_DIAGNOSTICS__', {
        configurable: true,
        value: { drawCount: 0, rafCount: 0, wrapperError: null, wrapperInstalled: false },
      });
      ...
      try {
        ...
        captureWindow.__JET_OG_CAPTURE_DIAGNOSTICS__.wrapperError = String(error);
      }
    });
    ...
    try {
      ...
      const state = await page.evaluate(() => ({
        ...
        diagnostics: (window as Window & { __JET_OG_CAPTURE_DIAGNOSTICS__?: unknown })
          .__JET_OG_CAPTURE_DIAGNOSTICS__,
      ...
      for (const specification of fontSpecifications) {
        const loaded = await Promise.race([
          ...
          new Promise<never>((_resolve, reject) => {
            setTimeout(() => reject(new Error(`FONT_LOAD_TIMEOUT:${specification}`)), 15_000);
          }),
        ]);
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  captureDefaultOpenGraphImage(process.argv.slice(2), repositoryRoot).catch((error: unknown) => {
