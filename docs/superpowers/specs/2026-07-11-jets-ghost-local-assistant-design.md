...
Conversation movement uses sticky follow within the conversation scroller and end sentinel, never the page body. Submission reveals the new user turn and assistant state immediately. After touch or pen submission blurs the composer, final positioning waits for visual-viewport resize activity to settle, with a bounded fallback. Streaming follows only while the reader remains within a small bottom threshold. A manual scroll above that threshold suspends following; later content below exposes an accessible compact **Jump to latest** control. Activating it restores following and reaches the end without focusing the composer, and a manual return to the bottom restores following automatically. Continuous following uses immediate scroller movement; the explicit jump may be smooth only when reduced motion is not requested. Existing in-flow bottom clearance keeps completed content and its collapsed source disclosure clear of the composer and navigation dock.
