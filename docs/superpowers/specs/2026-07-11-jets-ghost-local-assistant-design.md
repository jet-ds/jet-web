...
The genuinely content-sized, chrome-free status group lives in the left identity block directly below the version and Licenses metadata. It contains only the state dot and compact label, and its left edge aligns with the metadata text. It has no fixed or minimum width, placeholder or invisible sizer, border, background, rounded container, shadow, or padding. **Ready** and **Responding** share the same left edge, with the longer label growing rightward. The right-hand header group is actions-only and stays stable while status width changes independently. New session and Unload remain mounted in `ready`, `generating`, `cancelling`, `generation-error`, `resetting`, `reset-error`, `unloading`, and `unload-error`; New session is enabled only in `ready`, `reset-error`, and the supported `conversation-limit-reached` generation error, while Unload is enabled whenever cleanup can be requested or retried and disabled during `unloading`. At 320px, 430px, tablet, and desktop widths the identity and action groups remain uncrowded with zero horizontal overflow. The visual label is one of exactly six values and every lifecycle state maps deliberately:
