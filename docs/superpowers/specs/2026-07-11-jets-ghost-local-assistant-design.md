...
| Loading | Keep one honest indeterminate loading surface active with independent liveness copy and monotonic elapsed time. | No fabricated percentage and no visitor-facing Cancel or Unload action while LiteRT's current API cannot abort the active load. |
...
LiteRT-LM 0.14.0 does not expose an abort signal or trustworthy byte progress for WASM or model loading. Show a visibly traveling indeterminate segment, monotonic elapsed time, and visitor-facing liveness copy that changes every 12 seconds independent of the SDK's coarse technical phases. After a sustained wait, the copy explicitly says that the large local model can take several minutes. Reduced motion stops the segment animation while retaining the changing text and elapsed timer. Do not invent a percentage or imply that a transfer is making measurable progress.
...
4. **Loading** — changing truthful liveness copy, monotonic elapsed time, and a moving indeterminate indicator, with no unsupported cancel action.
