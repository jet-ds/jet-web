...
The loading surface exposes no Cancel or Unload action because the active LiteRT call cannot honor it. Generation Stop remains immediate through `conversation.cancel()`, and Unload becomes available after the model reaches a state where ordered cleanup can truthfully run. Route-unmount cleanup still invalidates the operation, suppresses Ready, and deletes any created engine once the pending call settles; full document close or reload terminates the document itself.
