...
- `/chatbot/` is the `200` qualification and production document; its canonical URL is `https://jetsanchez.com/chatbot/`, and platform trailing-slash normalization permanently redirects `/chatbot` to it with status `308`.
- Platform trailing-slash normalization permanently redirects `/tools/chatbot` to `/tools/chatbot/` with status `308`; one explicit legacy rule then permanently redirects `/tools/chatbot/` to `/chatbot/` with status `308`. Both Tools variants therefore resolve permanently to the canonical Ghost document through one coherent platform-plus-explicit chain.
...
| Loading | Keep one honest indeterminate loading surface active with a loading-only phase-in ghost, independently cycling themed headline, monotonic elapsed time, a reserved secondary reassurance slot, and a neutral **Cancel and reload** action. | No progress bar, ring spinner, fabricated percentage, in-place runtime cancellation, or Unload action while LiteRT's current API cannot abort the active load. **Cancel and reload** reloads the document at the browser boundary rather than entering application cleanup. |
...
Compatibility checking is cheap and reversible; it is not consent to download or allocate the model. Loading on route entry or first message is prohibited. Closing or reloading the document terminates its work at the browser boundary; a ClientRouter unmount during an in-flight LiteRT load invalidates the operation and performs ordered cleanup after the non-abortable API call settles, without claiming immediate cancellation.
...
Below that visual, preserve one stable content hierarchy throughout loading: the mono **LOADING ON THIS DEVICE** eyebrow; a large themed serif headline cycling every 12 seconds through **Haunting Jet's archive**, **Waking the ghost**, and **Feeding it ones and zeroes**; monotonic elapsed time; then a reserved one-line secondary-message slot. At 36 seconds, reveal **First load may take a few minutes.** only in that text-xs tertiary slot. At standard 320px-and-wider layouts, the broad slot holds this exact English copy on one line and reserves only one line height before the threshold, so its appearance does not replace or reset the themed headline or move the loading stack. Under zoom, enlarged text, localization, or unusually constrained widths, the slot may wrap and reflow naturally rather than clip or force one line. The reassurance is not a fabricated progress claim.

The phase-in visual and headline cycle are decorative and are not announced repeatedly; the separate factual lifecycle live region remains the accessible status. Reduced motion renders the centered ghost, faint afterimages, and binary particles as fixed layers while retaining the changing headline and elapsed timer. Do not invent a percentage or imply that a transfer is making measurable progress.

The loading surface exposes **Cancel and reload**, not an in-place SDK cancellation. It calls a document reload immediately and does not enter the application Unloading lifecycle or wait for the non-abortable LiteRT promise to settle. Generation Stop remains immediate through `conversation.cancel()`, and Unload becomes available after the model reaches a state where ordered cleanup can truthfully run. Route-unmount cleanup still invalidates the operation, suppresses Ready, and deletes any created engine once the pending call settles; full document close or reload terminates the document itself.

The activation surface owns one stable compact `text-xs` status-message slot above its action. It shows **This browser is ready for the local runtime** in `awaiting-consent` and replaces that text in place with the exact recoverable corpus error **Jet's Ghost could not load its published knowledge base.** in `load-error`. The focused recovery action references this same message; there is no duplicate error paragraph below the action. Reserve the normal rendered height for two lines at the narrowest supported mobile width and one line from 430px when the exact copy fits, preventing action or surrounding-layout movement between consent and error.

Render **Prompts stay here**, **No cloud history**, and **Session only** as natural-width, non-breaking items in a centered wrapping row rather than three equal columns. Complete items may move to a new line but must never overlap. Preserve the reviewed normal-mobile geometry; only below 370px, add `5xl` activation-main bottom clearance and use `text-sm`, `px-s`, and no wrapping on the load action so the complete label and action scroll fully above the fixed dock.

The genuinely content-sized, chrome-free status group lives in the left identity block directly below the version and Licenses metadata. A 48px Ghost mark with a 24px glyph top-aligns with the three-line identity stack at every supported width. The status contains only the state dot and compact label, and its left edge aligns with the metadata text. It has no fixed or minimum width, placeholder or invisible sizer, border, background, rounded container, shadow, or padding. **Ready** and **Responding** share the same left edge, with the longer label growing rightward. The right-hand header group is actions-only and stays stable while status width changes independently. New session and Unload remain mounted in `ready`, `generating`, `cancelling`, `generation-error`, `resetting`, `reset-error`, `unloading`, and `unload-error`; New session is enabled only in `ready`, `reset-error`, and the supported `conversation-limit-reached` generation error, while Unload is enabled whenever cleanup can be requested or retried and disabled during `unloading`. At 320px, 430px, tablet, and desktop widths the identity and action groups remain uncrowded with zero horizontal overflow. The visual label is one of exactly six values and every lifecycle state maps deliberately:
...
Focus behavior is based on actual interaction modality, not viewport width. Root capture preserves pointer type and distinguishes keyboard from mouse, touch, and pen. Pointer/tap submit blurs the textarea synchronously before generation. Every key event from a touch- or pen-focused composer preserves virtual-keyboard modality through Enter, while mouse focus followed by hardware Enter may switch to keyboard. Loading or resetting returns focus only when Load or New session was keyboard-initiated. Generation completion, cancellation, or generation-error recovery returns focus only when that message was keyboard-submitted; pointer/touch submissions remain blurred and no streaming state steals focus.

Conversation movement uses sticky follow within the conversation scroller and end sentinel, never the page body. Submission reveals the new user turn and assistant state immediately. After touch or pen submission blurs the composer, final positioning waits for visual-viewport resize activity to settle, with a bounded fallback. Streaming follows only while the reader remains within a small bottom threshold. A manual scroll above that threshold suspends following; later content below exposes an accessible compact **Jump to latest** control. Activating it immediately restores following and reaches the end without focusing the composer, and a manual return to the bottom restores following automatically. Immediate movement avoids confusing programmatic movement with a reader scroll-away and inherently respects reduced motion. Existing in-flow bottom clearance keeps completed content and its collapsed source disclosure clear of the composer and navigation dock.
...
The response parser accepts citations only from the selected-source allowlist. Unknown citation IDs are not rendered as links. Inline lookup remains exact by citation ID and source. The collapsed source disclosure for a completed or stopped turn derives only from validated citations, never selected or packed context. It deduplicates cited chunks by `canonicalUrl` in stable first-citation order, using the first cited ID, source title, and URL for each document. With zero cited documents the response renders no disclosure; a stopped turn may still render its separate **Stopped** label. No empty permanent source panel occupies the pre-conversation interface.

By default, the disclosure is collapsed and expansion state belongs to that assistant response. Its semantic button reads **1 source** or **N sources** with a chevron and uses `aria-expanded` plus `aria-controls` for a properly associated in-flow region and list. Expanded items are clean text links, not persistent source chrome: each shows the citation number and full document title with natural wrapping and no title truncation. The trigger preserves a minimum 44px touch target, visible keyboard focus, and a reduced-motion-safe state change. The control and list remain full-width-safe at 320px and 430px, align to the assistant column at tablet portrait and landscape sizes, and use a compact trigger plus controlled readable list width on desktop. Expansion changes only its response's in-flow height and never exposes uncited context.
...
4. **Loading** — a distinct phase-in ghost with inward binary particles, independently cycling themed headline, monotonic elapsed time, and reserved secondary reassurance, with no progress-shaped indicator or unsupported cancel action.
5. **Ready** — suggested questions derived from eligible source metadata and a free-form input.
6. **Generating** — streaming response and Stop action; the collapsed source disclosure appears only after validated citations exist for a completed or stopped response.
...
All button-like Ghost controls consume the same framework-neutral action taxonomy as the Astro site. Compact controls preserve a minimum 44×44px target; the prototype's primary lifecycle and consent controls define the documented immersive density at a 48×48px minimum and 12px radius. The shared recipe owns hover, focus-visible, disabled, reduced-motion, and forced-colors behavior. Lifecycle dots use explicit idle/ready/active status roles, while the composer Stop control uses explicit stop fill and foreground roles rather than assigning text tokens as backgrounds.
...
- focus after Load, message completion/cancellation/recovery, and New session follows recorded interaction modality: keyboard initiation restores composer focus, while mouse/touch/pen initiation remains blurred; a successful post-load requested Unload or cleanup-error recovery focuses “Check compatibility,” while failed cleanup leaves focus on its error action;
...
Route/navigation integration tests verify `/chatbot/` owns exact canonical and Open Graph URL `https://jetsanchez.com/chatbot/`, WebPage and SoftwareApplication IDs/URLs use that slashful base, and Production sitemap membership is exact. The Ghost dock item replaces Tools in interactive, no-script, and structured navigation with href `/chatbot/`; `/tools/` remains dormant and excluded; `/tools` normalizes to it without affecting `/toolshed/`; and production enforces the coherent route matrix: `/chatbot` normalizes to the `200` `/chatbot/` document, `/tools/chatbot` normalizes to `/tools/chatbot/`, and the one explicit legacy rule redirects `/tools/chatbot/` to `/chatbot/`.
...
| --- | --- |
| Route and IA | `/chatbot/` returns `200` with canonical `https://jetsanchez.com/chatbot/`; platform normalization returns exact `308`s from `/chatbot` to `/chatbot/` and `/tools/chatbot` to `/tools/chatbot/`; the sole explicit legacy rule returns exact `308` from `/tools/chatbot/` to `/chatbot/`; Ghost href `/chatbot/` replaces Tools in every navigation representation; `/tools/` remains dormant, noindexed, and excluded |
...
- `/chatbot` returns exact platform `308` to the canonical `200` `/chatbot/` document; `/tools/chatbot` returns exact platform `308` to `/tools/chatbot/`, whose sole explicit legacy rule returns exact `308` to `/chatbot/`; dormant `/tools/` remains noindexed and out of navigation and sitemap while `/tools` normalizes once and `/toolshed/` remains unaffected;
