...
The capture script refuses an output path inside the immutable baseline, requires the sanitized deployment's `gitSource.sha` to equal `--expected-commit`, and writes screenshots, `manifest.json`, `comparison.json`, and a copied sanitized `deployment.json` under the exact candidate output directory. In comparison mode it also sends Vercel's official `x-vercel-skip-toolbar: 1` automation header on every browser-context request; standalone production-baseline capture remains header-free. Compare home, blog index/detail, works index/detail, tools, and contact at both baseline viewports. Reject unexplained changes to palette, typography, Utopia spacing, Liquid Glass geometry, or Grainient appearance for visitors without reduced-motion preferences; record reviewed intentional differences in `comparison.json`. Preserve this directory for the release asset and then remove the private raw-input directory.
