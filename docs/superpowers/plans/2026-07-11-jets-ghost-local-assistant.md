...
Track actual interaction modality at the experience root, preserving mouse, touch, and pen pointer type instead of inferring from viewport. Pointer/tap submit must synchronously blur the textarea before generation. Classify touch-origin Enter as virtual-keyboard input; mouse-focused composer plus hardware Enter may switch to keyboard. Auto-focus after `loading -> ready` and `resetting -> ready` only for keyboard-initiated actions, and after generation completion/cancellation/recovery only for keyboard-submitted messages. Never focus during streaming. Keep late tokens and the completed response visible by scrolling the conversation scroller to its end sentinel without moving document focus or jumping the page.
...
- validated inline citation links plus document footer items derived only from that completed or stopped turn's validated citations, never packed/selected context; deduplicate by `canonicalUrl` in stable first-citation order and keep the first cited ID/title/URL;
- each unique cited document remains one rounded, max-width-safe, keyboard-focusable link whose non-shrinking source prefix stays on the first line beside a separate `min-w-0` title span; use CSS `line-clamp-2` on touch/mobile and `line-clamp-1` under the existing desktop/fine-pointer variant, start alignment, and contained overflow; the `aria-label` preserves the full source title and the native `title` attribute preserves the full source title, while the rendered text remains complete in the DOM with no JavaScript string truncation;
- no footer/source wrapper for a non-stopped turn with zero cited documents; a stopped turn may still render its label;
...
Formally verify the pre-message disclosure **Jet’s Ghost can make mistakes. Check cited sources.** immediately above the form on desktop and 430px touch: selecting a starter keeps it visible, first submit removes it without reserved space, failures do not restore it, and successful New session does. Use actual pointer and keyboard paths. Require pointer Load and New session not to focus the textarea; require pointer submit immediate blur through Ready, Responding, completion, cancellation, and recovery; classify touch-origin Enter as virtual-keyboard submission; require desktop keyboard Load, hardware Enter, and keyboard New session to retain/restore focus. Verify late tokens and completion remain visible above the composer/dock without document-focus movement or page jump.

At 430px touch, run a fake/static-corpus response with many selected context chunks and response **[S1]**, then require exactly one document source item. Confirm inline lookup is exact, uncited packed sources are absent, repeated cited chunks sharing `canonicalUrl` collapse to the first cited document, and distinct cited documents preserve first-citation order. After scrolling the conversation, require the source footer does not overlap the composer or dock and require zero horizontal/page overflow. This citation-footer regression is correctness, not polish. This is the formal regression for the Task 8 status/action, composer-metadata, reliability, modality, response-visibility, and citation contract; Task 8's flagged browser pass is only ad hoc evidence and does not satisfy this Task 10 requirement.

Use the exact long title **The Recursive Convergence Hypothesis: Emergent Sentience as a Structural Attractor of Recursive ASI** for the cited-source presentation regression. At 430px touch require the non-shrinking source prefix to remain visible on the first line, the title span to use `line-clamp-2`, and its height to stay within two rendered lines; at desktop/fine pointer require `line-clamp-1` and height within one rendered line. In both cases require one max-width-safe rounded link, complete title text in the DOM, the full title in `aria-label` and native `title`, a visible keyboard focus ring, no JavaScript truncation, no horizontal/page overflow, and no source-footer overlap with the composer or dock.

- [ ] **Step 3: Test cancellation and recovery**

Start a slow fake stream, press Stop, assert one partial response marked “Stopped,” submit a second question, and assert the second response completes once. Verify each response footer derives only from its validated citations, document items deduplicate by `canonicalUrl` in stable first-citation order, uncited selected context never renders, a zero-citation completed response has no footer wrapper, and no empty permanent source panel exists before generation.
