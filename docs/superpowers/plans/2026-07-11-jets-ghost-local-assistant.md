...
Use “Check compatibility” then “Load Jet's Ghost” as separate explicit actions. Preserve the final reviewed copy from the prototype where it is more specific than the generic sentence above. Unsupported state offers links to Blog and Works and no broken text input. While LiteRT provides neither abort nor trustworthy byte progress, decouple visitor-facing loading copy from its coarse phases: change the phrase every 12 seconds, cycle calm long-wait messages after the first sequence, explicitly note that a large local model can take several minutes, keep elapsed time monotonic, and move one indeterminate segment across its track. Reduced motion keeps the segment still while text and elapsed time continue. Show no Cancel or Unload action during Loading. Do not show a percentage, “Cancel and unload,” “Unload anytime,” or any other control/copy that promises in-place load cancellation; use **Session only** in the introduction summary instead.
...
Hold the fake activation in Loading for at least 40 seconds without completing it. Require monotonic elapsed time, the exact 12-second liveness-copy transitions, the several-minutes message, a segment that changes horizontal position over time, and no Cancel or Unload action. In a reduced-motion run, require the segment to stay fixed while copy and elapsed time continue changing. Also prove the ordinary post-load Unload state keeps elapsed time increasing while delayed cleanup is pending. These tests establish the interface contract only; they do not substitute for Task 11's real Gemma load.
