...
Place a genuinely content-sized, chrome-free status group before the stable header actions in the right flex row. Give it no fixed or minimum width, placeholder or invisible sizer, border, background, rounded container, shadow, or padding. Keep New session and Unload mounted in `ready`, `generating`, `cancelling`, `generation-error`, `resetting`, `reset-error`, `unloading`, and `unload-error`; derive explicit visibility and enabled-state booleans instead of conditionally mounting only in Ready. Enable New session only in `ready`, `reset-error`, and a `conversation-limit-reached` generation error. Keep Unload enabled for safe cleanup requests/retries and disable it during `unloading`. The stable actions own the right edge, so **Responding** grows leftward without moving either action; add no artificial gap beyond the normal flex `gap-2xs`. States without actions are exempt from neighboring-anchor stability. Visible values remain exactly **Not running**, **Checking**, **Load ready**, **Loading**, **Ready**, and **Responding**; map all lifecycle states deliberately and never append a percentage. Keep dot/text alignment fixed and crossfade the label with outgoing/incoming layers in one CSS-grid cell so both contribute intrinsic width until exit completes and the longer outgoing label is never clipped. Reduced motion renders only the current normal-flow grid item and swaps immediately. Render fuller `getLifecycleAnnouncement()` text in a separate visually hidden polite live region; the visible status group is not itself live.
...
At desktop and representative mobile widths, compare **Ready** with **Responding** and require each stable header action x/y/width/height to remain within `1px`; also require the brand x/y anchor to remain within `1px`. Verify the status is before New session then Unload in DOM/visual order, remains genuinely content-sized with **Ready** narrower than **Responding**, and expands leftward while action boxes stay fixed. Reject any fixed/min width, placeholder or invisible sizer, border, background, radius, shadow, or padding on status, and require no artificial gap beyond normal flex `gap-2xs`. Assert its computed border, background, shadow, radius, and padding are absent, zero, or transparent. Assert both actions remain mounted through every conversational lifecycle state, with New session and Unload enabled/disabled according to the Task 8 safety rules; states without actions are exempt from adjacent-anchor comparison. Assert the exact six-label allowlist, no percentage suffix, separate fuller polite live-region announcement, overlapping intrinsic crossfade layers, and immediate reduced-motion replacement. At 430px touch, require the keyboard hint computed display to be `none`, **Local only** to remain visible, metadata `justifyContent` to align naturally at the end, no blank placeholder, and zero horizontal overflow. On a desktop keyboard, require both metadata items visible on one line with space-between alignment.
