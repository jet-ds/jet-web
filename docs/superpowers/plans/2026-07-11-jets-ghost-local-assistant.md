...
`JETS_GHOST_PATHS.liteRtWasm` is the static same-origin directory emitted in Task 1. Passing it explicitly is mandatory because `Engine.create()` otherwise initializes the SDK singleton from `LiteRtLm.DEFAULT_WASM_PATH` on jsDelivr. LiteRT-LM `0.14.0` exposes no load abort or byte progress. Do not expose an in-place loading cancellation or Unload action that the API cannot honor. Expose a neutral **Cancel and reload** action that performs an immediate document reload without entering application Unloading state; this is explicitly a page restart, not a LiteRT abort. Route-unmount cleanup invalidates the operation, waits for the current API call to settle, deletes any engine that was created, calls `unloadLiteRtLm()`, and never enters Ready; closing or reloading the document terminates the page at the browser boundary.
...
For Loading, branch inside `AnimatedGhost` rather than reusing `ghostMotion.loading`. The result is an indeterminate phase-in animation, not measured progress. Wrap the complete loading content stack in `data-testid="loading-stack"`. Render a `data-testid="loading-phase-visual"` centered visual containing a `data-testid="loading-main-ghost"` main `Ghost`, two `data-testid="loading-ghost-afterimage"` copies, and four `data-testid="loading-inward-particle"` mustard `0`/`1` particles. The two afterimages loop for `2.4s`, staggered by `1.2s`, with scale keyframes `[0.88, 1.16, 1.45]` and opacity `[0, 0.28, 0]`. Use four particle paths with outer, curved-intermediate, and center coordinates: `[-30,-18] -> [-12,-20] -> [0,0]`, `[28,-20] -> [18,-8] -> [0,0]`, `[32,18] -> [10,16] -> [0,0]`, and `[-28,22] -> [-18,8] -> [0,0]`. Stagger particles by `0.6s`; use opacity `[0, 0.75, 0]`, scale `[0.75, 1, 0.45]`, and the same `2.4s` loop. Resolve the main ghost with opacity `[0.72, 1, 0.72]` and scale `[0.97, 1.03, 0.97]`; do not give it x/y translation. Retain the existing motion profiles for every non-loading state. Under reduced motion, render the same layers at fixed positions and restrained opacity with no repeated transform or opacity animation. Keep all phase-in layers `aria-hidden` through the decorative parent. Remove the old loading progress track and segment completely.

Below the phase-in visual, keep this exact hierarchy stable: mono **LOADING ON THIS DEVICE** eyebrow; a large themed serif headline cycling every 12 seconds through **Haunting Jet's archive**, **Waking the ghost**, and **Feeding it ones and zeroes** for the full lifecycle; monotonic elapsed time; then a reserved one-line secondary-message slot. At 36 seconds show **First load may take a few minutes.** only in that text-xs tertiary slot, without replacing or resetting the headline or moving the loading stack at normal tested widths. Make the slot broad enough for this exact English copy to remain one line at standard 320px-and-wider layouts and reserve only one line height before the threshold. Permit natural wrapping and reflow under zoom, enlarged text, localization, or unusually constrained widths. Keep the phase-in visual and decorative headline out of live regions and retain the separate factual lifecycle announcement. Show no progress bar, track, ring spinner, percentage, or Unload action during Loading. Add a neutral **Cancel and reload** button that calls the injected/default document-reload function immediately, never calls `ghost.unload()`, and never transitions to **Releasing this device**. Do not show “Cancel and unload,” “Unload anytime,” or any other control/copy that promises in-place load cancellation; use **Session only** in the introduction summary instead.

Use one compact `text-xs` `jets-ghost-activation-status` message slot above the activation action. It shows **This browser is ready for the local runtime** in `awaiting-consent` and the exact error **Jet's Ghost could not load its published knowledge base.** in `load-error`; the focused **Return to load** action uses `aria-describedby` to reference the same slot. Remove the separate error paragraph below the action. Reserve two message lines at the narrowest supported mobile width and one from 430px when the exact English strings fit, then verify that the action and surrounding activation layout do not move between the ready and error messages.

Replace the activation disclosure's three equal grid columns with a centered natural-width `flex-wrap` row using `gap-x-s` and `gap-y-2xs`. Keep each icon-and-label item non-breaking, allowing only whole items to wrap. At 320px require two clean rows with no overlap or horizontal overflow; at 430px require one line where the three items fit. Do not alter reviewed normal-mobile geometry to solve the narrow case: apply `max-[369px]:pb-5xl` only to the activation main and `max-[369px]:text-sm`, `max-[369px]:px-s`, and `max-[369px]:whitespace-nowrap` only to its load action, so the complete one-line label can scroll fully clear of the fixed dock.

Place a genuinely content-sized, chrome-free status group in the left identity block directly below the version and Licenses metadata, with its left edge aligned to that metadata text. A 48px Ghost mark with a 24px glyph top-aligns with the three-line identity stack at every supported width. Give the status no fixed or minimum width, placeholder or invisible sizer, border, background, rounded container, shadow, or padding. Make the right-hand group actions-only. Keep New session and Unload mounted in `ready`, `generating`, `cancelling`, `generation-error`, `resetting`, `reset-error`, `unloading`, and `unload-error`; derive explicit visibility and enabled-state booleans instead of conditionally mounting only in Ready. Enable New session only in `ready`, `reset-error`, and a `conversation-limit-reached` generation error. Keep Unload enabled for safe cleanup requests/retries and disable it during `unloading`. **Responding** grows rightward from the same left edge as **Ready**, while the independent right-hand actions stay stable. Visible values remain exactly **Not running**, **Checking**, **Load ready**, **Loading**, **Ready**, and **Responding**; map all lifecycle states deliberately and never append a percentage. Crossfade labels so the outgoing label leaves intrinsic layout immediately as an overlay while the current label remains in normal flow and defines the group's width from its first visible frame. Reduced motion renders only the current normal-flow grid item and swaps immediately. Render fuller `getLifecycleAnnouncement()` text in a separate visually hidden polite live region; the visible status group is not itself live.

Make composer metadata input-aware. Hide the keyboard hint **Enter sends · Shift+Enter newline** with `display: none` until both the explicit desktop/tablet width and fine-pointer media conditions match. Keep **Local only** visible at every width. Use natural mobile `justify-end` alignment without a blank placeholder or reserved left column; on a desktop keyboard, show both items on one nowrap line with `justify-between`. Source/design tests inspect these exact responsive classes and reject invisible placeholders or spacer elements.

Before the first actual submit, render the exact visible copy **Jet’s Ghost can make mistakes. Check cited sources.** immediately above the Composer form inside the composer stack, separate from starter prompts and metadata. Suggested-question selection only fills/focuses the draft and leaves the disclosure visible. First submit removes it with no placeholder or reserved space. Restore it only after successful New session or requested Unload; preserve its removed state through generation, reset, and unload failures.

Track actual interaction modality at the experience root, preserving mouse, touch, and pen pointer type instead of inferring from viewport. Pointer/tap submit must synchronously blur the textarea before generation. Preserve touch/pen modality for every composer key event after touch/pen focus through virtual-keyboard Enter; mouse-focused composer plus hardware Enter may switch to keyboard. Auto-focus after `loading -> ready` and `resetting -> ready` only for keyboard-initiated actions, and after generation completion/cancellation/recovery only for keyboard-submitted messages. Never focus during streaming.

Use intentional sticky follow within the conversation scroller and end sentinel rather than unconditional scroll-to-bottom on each turn update. Submission reveals the new user and assistant state. Touch/pen submission blurs first, then performs final positioning after visual-viewport resize activity settles, with a bounded fallback. Follow delayed chunks only while the reader is within a small bottom threshold. Manual scroll-away suspends following; later content below reveals an accessible compact **Jump to latest** control. Activating it immediately restores follow without focusing the composer, and a manual return to the bottom restores follow automatically. Immediate jump movement avoids treating programmatic scroll frames as reader intent and inherently respects reduced motion. Never scroll the page body.
...
- validated inline citation links plus a collapsed source disclosure derived only from that completed or stopped turn's validated citations, never packed/selected context; deduplicate by `canonicalUrl` in stable first-citation order and keep the first cited ID/title/URL;
- default the per-response disclosure to collapsed; its semantic button reads **1 source** or **N sources** with a chevron and uses `aria-expanded` plus `aria-controls` for an associated in-flow region/list; do not auto-expand at completion;
- expanded items are clean keyboard-focusable text links that show citation number and full document title with natural wrapping and no title truncation; keep the trigger at least 44px tall on touch, keep the control/list within the response column, and make expansion reduced-motion-safe;
- render no disclosure for zero cited documents; keep a stopped response's label separate;
...
- focus follows interaction modality after Load, message completion/cancellation/recovery, and New session: keyboard initiation restores the input, while pointer/touch/pen leaves it blurred; successful post-load requested Unload and cleanup-error Unload recovery focus “Check compatibility,” while failed cleanup preserves session UI state and error-action focus;
- streamed and completed responses stay visible through the conversation scroller/end sentinel without stealing focus;
...
At desktop and representative mobile widths, compare **Ready** with **Responding** and require each stable header action x/y/width/height to remain within `1px`; also require the brand x/y anchor to remain within `1px`. Verify the status lives in the identity block below the version/license row, is absent from the actions-only group, remains genuinely content-sized with **Ready** narrower than **Responding**, and preserves the same left edge while growing rightward. Require the status x and metadata text x to remain within `1px`. Reject any fixed/min width, placeholder or invisible sizer, border, background, radius, shadow, or padding on status. Assert its computed border, background, shadow, radius, and padding are absent, zero, or transparent. Capture **Load ready** and **Responding** immediately after each becomes current and again after the exit duration; require label and status x/y/width/height to remain within `1px`, proving the outgoing label no longer participates in intrinsic layout. Assert both actions remain mounted through every conversational lifecycle state, with New session and Unload enabled/disabled according to the Task 8 safety rules. Assert the exact six-label allowlist, no percentage suffix, separate fuller polite live-region announcement, overlay crossfade, and immediate reduced-motion replacement. At 320px, 430px, tablet, and desktop widths, require the identity, metadata, status, and action groups to remain inside the header without crowding and with zero horizontal overflow. At 430px touch, require the keyboard hint computed display to be `none`, **Local only** to remain visible, metadata `justifyContent` to align naturally at the end, and no blank placeholder. On a desktop keyboard, require both metadata items visible on one line with space-between alignment.

Formally verify the pre-message disclosure **Jet’s Ghost can make mistakes. Check cited sources.** immediately above the form on desktop and 430px touch: selecting a starter keeps it visible, first submit removes it without reserved space, failures do not restore it, and successful New session does. Use actual pointer and keyboard paths. Require pointer Load and New session not to focus the textarea; require pointer submit immediate blur through Ready, Responding, completion, cancellation, and recovery; classify touch-origin Enter plus preceding virtual-keyboard character events as touch modality; require desktop keyboard Load, hardware Enter, and keyboard New session to retain/restore focus.

With long delayed output that overflows at 320px, 430px, tablet portrait and landscape, and desktop widths, verify sticky follow from initial submit through completion. Touch submission must blur first and perform final scroller positioning only after visual-viewport settling; desktop Enter retains its keyboard focus path. While streaming near the bottom, each chunk follows. After manual scroll-away beyond the threshold, later chunks must not change scroll position and an unfocused, keyboard-operable **Jump to latest** control appears. Activating it restores follow without focusing the composer; manually returning to the bottom does the same. Reduced motion uses immediate programmatic movement. No path scrolls the page body, and the completed response plus collapsed source disclosure remain clear of composer and dock.

Hold the fake activation in Loading for at least 40 seconds without completing it. Require monotonic elapsed time and the exact 12-second themed-headline sequence, including its independent return to **Haunting Jet's archive** at 36 seconds. At 320px, 430px, tablet, and desktop normal layouts, require the reserved one-line secondary slot and overall `loading-stack` bounding boxes at 24 and 36 seconds to remain unchanged within `1px`, while **First load may take a few minutes.** appears only in the slot at 36 seconds. Confirm that the exact copy is one line at standard 320px-and-wider layouts, while a zoom/enlarged-text case may wrap and reflow without clipping.

Require exactly two afterimages and four inward particles, no loading progress track/segment/ring, and no x/y translation on the main loading ghost. Over at least `450ms`, require a phase-in afterimage or inward particle transform/opacity to change under normal motion and remain fixed under reduced motion. Verify the existing non-loading ghost profiles remain intact. Require no live-region semantics on the phase-in visual or decorative headline, require **Cancel and reload**, and require no Unload action. Inject a reload spy to prove the button requests one document reload without calling runtime/repository unload or rendering **Releasing this device**. Also prove the ordinary post-load Unload state keeps elapsed time increasing while delayed cleanup is pending. Capture and inspect dark/light Loading screenshots at 320px, 430px, tablet portrait/landscape, and desktop. These tests establish the interface contract only; they do not substitute for Task 11's real Gemma load.

In the activation-surface browser regression, force a recoverable corpus failure after consent and compare the activation-status slot plus action geometry with the preceding ready-for-runtime state. At 320px allow the reserved two-line message height; at 430px and desktop require the exact readiness and corpus-error strings to fit their reserved one-line height. Require the same stable status-slot identity, one visible error string, a focused **Return to load** button described by the slot, and no separate error paragraph. Hold slot and action y/height/center-x plus surrounding hero geometry within `1px`; natural content widths may differ.

At 430px touch, run a fake/static-corpus response with many selected context chunks and response **[S1]**, then require exactly one document source item behind a default-collapsed **1 source** trigger. Confirm inline lookup is exact, uncited packed sources are absent, repeated cited chunks sharing `canonicalUrl` collapse to the first cited document, and distinct cited documents preserve first-citation order behind count-aware **N sources** copy. Require no control for zero citations, independent open/closed state per response, and no automatic expansion at completion. After scrolling and expansion, require the source disclosure does not overlap the composer or dock and require zero horizontal/page overflow. This disclosure regression is citation correctness, not polish. This is the formal regression for the Task 8 status/action, composer-metadata, reliability, modality, sticky-follow, response-visibility, and citation contract; Task 8's flagged browser pass is only ad hoc evidence and does not satisfy this Task 10 requirement.

Use the exact long title **The Recursive Convergence Hypothesis: Emergent Sentience as a Structural Attractor of Recursive ASI** for the expanded disclosure regression. At 320px and 430px require a full-width-safe 44px trigger, naturally wrapped full document title, no title truncation, zero horizontal overflow, and no overlap with composer or dock. At tablet portrait and landscape widths require alignment with the assistant response column rather than arbitrary stretching. At desktop require a compact trigger, controlled readable list width, precise hover/focus states, and no layout change outside the response's in-flow expansion. Capture collapsed and expanded light/dark screenshots and verify `aria-expanded`, `aria-controls`, associated region/list semantics, count-aware **1 source** or **N sources** copy, full accessible link names, keyboard operation, stable first-citation ordering, and reduced-motion behavior.

- [ ] **Step 3: Test cancellation and recovery**

Start a slow fake stream, press Stop, assert one partial response marked “Stopped,” submit a second question, and assert the second response completes once. Verify each collapsed source disclosure derives only from its validated citations, document items deduplicate by `canonicalUrl` in stable first-citation order, uncited selected context never renders, a zero-citation completed response has no disclosure, and no empty permanent source panel exists before generation.

- [ ] **Step 4: Test unsupported, exhaustion, and failure states**

Cover no WebGPU, model load failure, corpus version mismatch, corpus-index mismatch, generation failure, conversation exhaustion, and recovery. For exhaustion, verify the complete visible transcript remains, no create/generate call is logged, and “Start new session” resets the conversation; keyboard initiation returns focus to an empty enabled input while pointer initiation does not. No unsupported state renders an enabled chat input.
...
Run axe on introduction, ready, response, and error states. Assert the separate fuller lifecycle status region exists, the compact chrome-free status group is not itself `aria-live`, streamed response is not itself `aria-live`, and all actions are keyboard reachable.
