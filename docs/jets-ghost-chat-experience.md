...
| Loading | Show independent liveness copy, a genuinely moving indeterminate indicator, and monotonic elapsed time while the non-abortable LiteRT call is pending. | No fabricated percentage and no Cancel or Unload action that the runtime cannot honor. Closing or reloading the document remains the browser-owned way to terminate the page and its work. |
...
LiteRT-LM 0.14.0 exposes neither trustworthy byte progress nor in-place load cancellation. During loading, the main phrase therefore changes every 12 seconds rather than remaining coupled to a long technical phase, the indeterminate segment travels visibly across its track, and elapsed time continues monotonically. Reduced motion keeps the segment still while the changing text and timer preserve liveness. The screen offers no Cancel or Unload control until loading has completed. It never invents a percentage, and no percentage is appended to the header status group.
