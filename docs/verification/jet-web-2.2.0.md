...
- Cache reuse must prove zero model-network requests on the warm path. The cold run uses a closed dock to isolate model behavior; one warm cache-reuse pass uses the visible dock to couple the separately owned dock and runtime evidence without treating cache storage as inference performance.
