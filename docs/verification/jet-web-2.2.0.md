...
## Task 12 final suite contract audit

All 60 test, fixture, setup, manual, and support paths present at audit start were inventoried and read as one system. The retained suite owns observable product, accessibility, privacy, security, resource-lifecycle, compatibility, legal-distribution, build-artifact, SEO, and deployment behavior. Overlapping search, source-payload, corpus, structured-data, Grainient, lifecycle-animation, and metadata cases were consolidated at their narrowest durable owner. The audit removed content snapshots, annotation-shape and fake-runtime ordinal checks, exact private call and operation choreography, source-layout checks already owned by emitted/deployed behavior, and developmental CSS/layer assertions. Two fully redundant unit files were removed; the final inventory contains 58 paths. The unit suite moved from 49 files / 583 tests to 47 files / 539 tests.

The conversation-exhaustion hook test now derives an exhausted turn from the measured runtime token count and proves rejection before retrieval, prompt assembly, or generation. The cleanup elapsed-time owner uses a controlled clock instead of waiting 1.1 seconds of runner wall time. No production owner change was required.

Scheduled GitHub runs `30393909284` and `29952278909` both failed only the mobile Chromium ClientRouter cleanup case on main commit `0d59dc9210f38f5ab32dd8636604563bef8024a3` under Node `24.18.0`. Each first attempt compared the private fake-runtime ordinal before the re-entered React island replaced the window audit; the stale and new values could both be `1`, and each retry passed. The replacement asserts the durable boundary: every prior resource is cleaned exactly once, the re-entered transcript is empty, and the new controller completes compatibility, load, session creation, response, and citation behavior without prior-state leakage. It passed 20/20 desktop and mobile repetitions with zero retries, then passed in the complete browser matrix. A later audit rerun also exposed a retained crossfade owner clicking its server-rendered compatibility control before island hydration; the test now synchronizes on fixture installation and passed 10/10 zero-retry repetitions plus the final matrix. CI retries and `failOnFlakyTests` remain unchanged.

Final Node `24.18.0` verification passed `format:check`, `lint`, the complete `verify` gate (47 unit files / 539 tests), `verify:build-purity`, and `git diff --check`. The complete browser matrix passed 249 tests with 19 intentional project skips and no retries or flakes. The manual real-model and physical-phone suites were read but not run or changed in this audit; the failed physical qualification and unreleased status above remain unchanged.

### Suite-review correction before the approved phone rerun

The follow-up suite review retained the 58-path and 47-file audit disposition above while correcting the previously stale manual runner. `warm-resume` now attaches to the already running Chrome profile over an explicit CDP endpoint, requires the exact pinned Cache Storage namespace and key to expose a readable status-`200` body with the declared `2,008,432,640` bytes, skips the cold activation path, rejects any model-network request or cache write during warm activation, and preserves the committed cache through Unload unless the operator explicitly sets `EGREGORE_REMOVE_MODEL_AFTER_QUALIFICATION=1` for final removal. The checked-in cache-preserving entry point is `EGREGORE_CDP_ENDPOINT=http://127.0.0.1:9222 npm run qualify:egregore:warm`; the local instrumented build remains required, so `REAL_MODEL_BASE_URL` is intentionally unset for the matching phone confirmation.

The warm interaction order is now frozen as Recursive Convergence, **Who is Jet?**, **What does Jet do?**, Digital Squad Timesheet, Claude native installation, and the unsupported private-note question. Those turns accumulate in one LiteRT conversation. The qualification-only audit records count-only conversation creation and actual token checkpoints after session creation and every completed turn, requires strictly growing counts, exercises Stop and recovery without a replacement, then proves **New session** performs one reset and creates exactly one replacement on the first post-reset send. The active replacement—not an unused new session—is unloaded. Successful Unload must expose one WebGPU device-destroy event, one retained-device-reference-clear event, one runtime-unloaded event, a coherent browser surface, and the still-readable disk cache. No prompts, responses, device identifiers, or private data enter that instrumentation.

The collection-filter browser owner now activates every visible Blog tag and Works type and compares the count in the visible control label with the rendered published cards. It does not inspect the cards' private `data-filter-values` payload or depend on hard-coded collection totals.

This is corrected harness readiness only. The cache-only accumulated-conversation confirmation has **not** been run after this correction and still requires the approved bounded Xiaomi 14T/ADB evidence window. Therefore the failed disposition in this record remains in force: no warm token-growth, process-memory, KGSL, thermal, termination, device-cleanup, or final qualification result is claimed, and no version, deployment, release, or Search Console step is authorized.

Follow-up deterministic verification on Node `24.18.0` passed the complete `verify` gate (47 unit files / 543 tests), `verify:build-purity`, and the complete browser matrix (249 passed, 19 intentional project skips, no retries or flakes). The collection-filter owner also passed 20/20 desktop and mobile repetitions. These local results verify the corrected test and qualification contracts only; they do not replace the pending physical warm-resume qualification described above.
