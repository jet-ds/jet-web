...
async function fixture(
  ...
  const documentNumbers = [...new Set(specs.map((spec) => spec.document))].sort(
    (a, b) => a - b,
  );
  const documents: KnowledgeDocument[] = documentNumbers.map(
    (number, index) => {
  ...
  const documentOrder = new Map(
    documents.map((document) => [document.id, document.order]),
  );
  const sectionKeys = [
    ...new Set(specs.map((spec) => `${spec.document}:${spec.section}`)),
  ].sort((left, right) => {
  ...
  const sections: KnowledgeSection[] = sectionKeys.map((key) => {
    ...
    const first = specs.find(
      (spec) => spec.document === document && spec.section === section,
    )!;
  ...
  const chunks: KnowledgeChunk[] = [...specs]
    .sort(
      (left, right) =>
        (documentOrder.get(`blog:document-${left.document}` as DocumentId) ??
          0) -
          (documentOrder.get(`blog:document-${right.document}` as DocumentId) ??
    ...
    .map((spec) => {
      ...
      return {
        ...
        order: spec.order,
        contentHash: chunkId(spec.document, spec.section, spec.order)
          .split(':')
          .at(-2)!,
  ...
  const fullCorpusKnowledgeTokens =
    serializeSourcePayload(payload).estimatedTokens;
...
function fixedResults(
  ...
  vi.spyOn(knowledgeBase.searchIndex, 'search').mockReturnValue(
    values.map(([id, score]) => ({
      id,
      score,
      terms: ['term'],
...
describe('deterministic MiniSearch rank and pack', () => {
  it('preserves MiniSearch scores from all evaluated boosted fields', async () => {
    ...
    expect(
      result.sources.every(
        (source) => source.rankingScore === raw.get(source.chunkId),
      ),
    ).toBe(true);
  });
  ...
  it('orders direct and adjacent candidates deterministically and deduplicates nominations', async () => {
    ...
    expect(new Set(result.sources.map((source) => source.chunkId)).size).toBe(
      4,
    );
  ...
  it('expands only immediate same-section neighbors and fails closed on corrupt lookups', async () => {
    ...
    fixedResults(knowledgeBase, [
      ['blog:unknown#section:dead:0' as ChunkId, 1],
    ]);
    expect(() =>
      rankAndPackContext({
        query: 'term',
  ...
  it('uses stable chunk ID after equal scores, reasons, and explicit orders', async () => {
    ...
    knowledgeBase.documentsById = new Map([
      [
        first.split('#')[0] as DocumentId,
        { ...knowledgeBase.documentsById.get('blog:document-0')!, order: 0 },
      ],
      [
        second.split('#')[0] as DocumentId,
        { ...knowledgeBase.documentsById.get('blog:document-1')!, order: 0 },
      ],
    ]);
    fixedResults(knowledgeBase, [
      [second, 3],
      ...
      [
        {
          ...
          section: 0,
          order: 0,
          text: 'private-looking source text',
        },
      ],
      { forceOversized: true },
    );
