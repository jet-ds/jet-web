...
import { normalizeMdx } from '../../../src/features/egregore/corpus/normalize';
...
Usage details.`);
    ...
    expect(
      sections.map(({ heading, headingPath, order }) => ({
        heading,
        headingPath,
        order,
      })),
      ...
      {
        heading: 'Shell',
        headingPath: ['Install', 'Linux', 'Shell'],
        order: 3,
      },
...
`);
    ...
    expect(sections[0].text)
      .toBe(`Paragraph with *emphasis*, **strength**, [a link](https://example.com), and \`inline code\`.
