...
function failureCodes(chain: ModelDeliveryHop[]): string[] {
  return validateModelDeliveryChain(chain, EGREGORE_MODEL).failures.map(
    ({ ruleCode }) => ruleCode,
  );
}
...
describe('model delivery origin policy', () => {
  it('accepts only HTTPS default-port origins on boundary-safe trusted hostnames', () => {
    ...
    expect(isTrustedModelOrigin('https://huggingface.co/model', policy)).toBe(
      true,
    );
    expect(isTrustedModelOrigin('https://cdn.hf.co/model', policy)).toBe(true);
    expect(
      isTrustedModelOrigin('https://edge.cdn.hf.co:443/model', policy),
    ).toBe(true);
    expect(isTrustedModelOrigin('http://cdn.hf.co/model', policy)).toBe(false);
    expect(isTrustedModelOrigin('https://cdn.hf.co:444/model', policy)).toBe(
      false,
    );
    expect(
      isTrustedModelOrigin('https://user:password@cdn.hf.co/model', policy),
    ).toBe(false);
    expect(
      isTrustedModelOrigin('https://cdn.hf.co.example.com/model', policy),
    ).toBe(false);
    expect(isTrustedModelOrigin('https://evilcdn.hf.co/model', policy)).toBe(
      false,
    );
  ...
  it('accepts exact and boundary-safe Xet delivery origins', () => {
    ...
    expect(isTrustedModelOrigin('https://xethub.hf.co/model', policy)).toBe(
      true,
    );
    expect(
      isTrustedModelOrigin('https://cas-bridge.xethub.hf.co/model', policy),
    ).toBe(true);
  });
  ...
  it('rejects Xet delivery lookalikes', () => {
    ...
    expect(
      isTrustedModelOrigin('https://xethub.hf.co.example.com/model', policy),
    ).toBe(false);
    expect(isTrustedModelOrigin('https://evilxethub.hf.co/model', policy)).toBe(
      false,
    );
  });
});
...
describe('model delivery chain validation', () => {
  it('requires the first request URL to equal the revision pin byte-for-byte', () => {
    const changedInitialUrl = terminalHop(
      `${PINNED_URL}?Policy=provider-change`,
    );
  ...
  it('resolves relative redirect locations against the current trusted URL', () => {
    const relativeTarget = new URL(
      '/signed/artifact?Key=changed&Signature=secret',
      PINNED_URL,
    ).href;
    const result = validateModelDeliveryChain(
      [
        {
          ...
          response: {
            status: 302,
            location: '/signed/artifact?Key=changed&Signature=secret',
          },
        },
  ...
  it('requires locations, rejects loops, and requires a successful terminal response', () => {
    ...
    expect(
      failureCodes([terminalHop(PINNED_URL, { response: { status: 503 } })]),
  ...
  it('accepts zero through five redirects and rejects the sixth', () => {
    for (
      let redirectCount = 0;
      redirectCount <= EGREGORE_MODEL.maxRedirects;
      redirectCount += 1
    ) {
      const result = validateModelDeliveryChain(
        deliveryChain(redirectCount),
        EGREGORE_MODEL,
      );
  ...
  it('allows provider delivery details to change within the approved boundary', () => {
    ...
    changed[changed.length - 2].response.location =
      changed[changed.length - 1].request.url;
        ...
        [
          terminalHop(PINNED_URL, {
            request: {
              ...
              method,
              headers:
                method === 'GET' ? { Range: 'bytes=0-65535' } : undefined,
              credentials: 'omit',
            },
          }),
        ],
...
describe('model artifact stream qualification', () => {
  ...
  it.each([
    ...
    [
      'mutated',
      Buffer.concat([Buffer.of(artifact[0] ^ 1), artifact.subarray(1)]),
    ],
  ])(
    'rejects a %s stream regardless of provider metadata',
    async (_label, bytes) => {
...
describe('model delivery diagnostics', () => {
  it('retains only the sanitized qualification projection', async () => {
    const signedUrl =
      'https://cdn.hf.co/transient/private?Signature=signature-sentinel&Policy=policy-sentinel';
    const validation = validateModelDeliveryChain(
      [
        {
          request: { url: PINNED_URL, method: 'GET' },
    ...
    const artifactResult = await verifyModelArtifactStream(
