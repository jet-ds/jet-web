...
import { rankAndPackContext } from '../../../src/features/jets-ghost/selection/rankAndPack';
import {
  buildSearchIndexArtifact,
  loadSearchIndex,
  MINISEARCH_OPTIONS,
} from '../../../src/features/jets-ghost/selection/searchIndex';
import type { ContextBudget } from '../../../src/features/jets-ghost/selection/types';
...
vi.mock('../../../src/features/jets-ghost/sourcePayload', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../src/features/jets-ghost/sourcePayload')>();
  return {
    ...actual,
...
function budget(knowledgeLimit: number): ContextBudget {
  return { ...JETS_GHOST_CONTEXT, knowledgeLimit };
}
