...
describe('content SEO metadata', () => {
  ...
  it('gives the research work a compact SEO title without replacing its paper title', () => {
    const parsed = worksSchema.parse(
      frontmatter('src/data/works/recursive-convergence-hypothesis.mdx'),
    ) as ParsedContentMetadata;

    expect(parsed.title).toBe(
      'The Recursive Convergence Hypothesis: Emergent Sentience as a Structural Attractor of Recursive ASI',
    );
    expect(parsed.seoTitle).toBe(
      'Recursive Convergence Hypothesis: AI Sentience',
    );
    expect(
      formatTitle(parsed.seoTitle ?? parsed.title).length,
    ).toBeLessThanOrEqual(60);
  ...
  it('gives Broad Reach, Uneven Depth compact search metadata without replacing its paper title', () => {
    const parsed = worksSchema.parse(
      frontmatter('src/data/works/broad-reach-uneven-depth.mdx'),
    ) as ParsedContentMetadata;
    ...
    expect(parsed.seoTitle).toBe(
      'Philippine AI Diffusion Across Three Systems',
    );
    expect(
      formatTitle(parsed.seoTitle ?? parsed.title).length,
    ).toBeLessThanOrEqual(60);
  ...
  it('gives the Timesheet work concise, complete search metadata', () => {
    const parsed = worksSchema.parse(
      frontmatter('src/data/works/digital-squad-timesheet.mdx'),
    ) as ParsedContentMetadata;

    expect(parsed.title).toBe('Digital Squad Timesheet');
    expect(parsed.seoTitle).toBe(
      'Digital Squad Timesheet: Operations Platform',
    );
  ...
  it.each([
    ...
    },
    {
      ...
      kind: 'work',
      seoDescription:
        'A cross-platform measurement audit finding that Philippine generative-AI standing changes materially across Microsoft, OpenAI, and Anthropic telemetry.',
    },
  ])(
    'records an explicit compact SEO description for $path',
    ({ path, kind, seoDescription }) => {
